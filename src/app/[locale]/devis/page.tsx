...
function QuoteForm() {
  ...
  const stepsAviation = [t("steps.flight"), t("steps.details"), t("steps.contact"), "Confirmation"];
  const stepsYacht = [t("steps.flight"), t("steps.details"), t("steps.contact"), "Confirmation"];
          ...
          {!submitted ? (
            <div className="glass-dark" style={{ padding: "clamp(24px, 4vw, 40px)" }}>
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {currentStep === 0 && !isYacht && (
                    <div style={colFlex()}>
                      <h3 style={stepTitleCSS}>{t("steps.flight")}</h3>

                      {/* Flight type */}
                      <div>
                        <label style={labelCSS}>{t("form.tripType")}</label>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "12px" }}>
                          {[{ v: "one-way", l: t("form.oneWay") }, { v: "round-trip", l: t("form.roundTrip") }, { v: "multi", l: t("form.multiCity") }].map((opt) => (
                            <button key={opt.v} onClick={() => update("type", opt.v)} style={optionBtn(form.type === opt.v)}>
                              {opt.l}
                            </button>
                          ))}
                        </div>
                  ...
                  {currentStep === 1 && !isYacht && (
                    <div style={colFlex()}>
                      <h3 style={stepTitleCSS}>{t("steps.details")}</h3>
                      ...
                      <div>
                        ...
                        <select value={form.category} onChange={(e) => update("category", e.target.value)} style={selectCSS}>
                          ...
                          {fleetCats.map((c) => (
                            <option key={c.slug} value={tData(c.name, locale)} style={{ background: "#132A3A" }}>{tData(c.name, locale)}</option>
                          ))}
                        </select>
                      </div>
                      ...
                      <div style={colFlex("12px")}>
                        <CheckboxField checked={form.pets} onChange={(v) => update("pets", v)} label={t("options.pets")} />
                        <CheckboxField checked={form.specialLuggage} onChange={(v) => update("specialLuggage", v)} label={t("options.oversizedLuggage")} />
                      </div>
                  ...
                  {currentStep === 2 && (
                    <div style={colFlex()}>
                      <h3 style={stepTitleCSS}>{t("steps.contact")}</h3>

                      <div style={gridTwoCol}>
                        <div>
                          <label style={labelCSS}>{t("form.lastName")}</label>
                          <input type="text" value={form.lastName} onChange={(e) => update("lastName", e.target.value)} style={inputCSS} />
                        </div>
                        <div>
                          <label style={labelCSS}>{t("form.firstName")}</label>
                  ...
                  {currentStep === 3 && (
                    <div style={colFlex()}>
                      <h3 style={stepTitleCSS}>{t("form.submit")}</h3>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: "40px" }}>
                ...
                >
                  {currentStep === 3 ? (isSubmitting ? "..." : t("form.submit")) : "Next"}
                </button>
              </div>
            </div>
          ) : (
